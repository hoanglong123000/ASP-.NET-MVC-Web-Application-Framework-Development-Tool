...
namespace Service.Education.Executes.Base
{
    ...
    {
        public TradeHistorieViewModel TradeHistorieOne(int id)
        {
            CheckDbConnect();
            var item = Context.Database.SqlQuery<TradeHistorieViewModel>("SELECT TOP 1 * from TradeHistories as T WHERE T.Id = " + id).FirstOrDefault();
            /*if(item != null)
            {
