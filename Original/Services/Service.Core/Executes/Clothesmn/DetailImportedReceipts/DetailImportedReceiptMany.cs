...
namespace Service.Education.Executes.Base
{
    ...
    {
        public QueryResult<DetailImportedReceiptViewModel> DetailImportedReceiptMany(int id, SearchDetailImportedReceiptModel model, OptionResult optionResult)
        {
            /*if (model.Cache)
            {*/
            ...
            return data;*/
            /* }*/
            return DetailImportedReceiptData(model, optionResult, id);
        }

        private QueryResult<DetailImportedReceiptViewModel> DetailImportedReceiptData(SearchDetailImportedReceiptModel model, OptionResult optionResult, int id)
        {
            CheckDbConnect();
            IQueryable<DetailImportedReceipt> q = Context.DetailImportedReceipts.Where(x => x.Status >= 0 && x.CouponId == id);
            ...
            var r = q.Select(x => new DetailImportedReceiptViewModel
            {
                Id = x.Id,
                Status = x.Status,
            ...
            var result = new QueryResult<DetailImportedReceiptViewModel>(r, optionResult);
