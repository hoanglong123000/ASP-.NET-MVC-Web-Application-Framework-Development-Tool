...
namespace Service.Education.Executes.Base
{
    ...
    {
        public DetailImportedReceiptViewModel DetailImportedReceiptOne(int id)
        {
            CheckDbConnect();
            var item = Context.Database.SqlQuery<DetailImportedReceiptViewModel>("SELECT * FROM DetailImportedReceipts WHERE CouponId = " + id).FirstOrDefault();
           /* if(item != null)
            {
